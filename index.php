        ...
        case 'blog':
            ...
            if ($db_total_records > 0 && $page_total > 1) {
                ...
                for ($tt=1; $tt <= $page_total; $tt++) {
                    ...
                    $t .= '<a href="' . $sn . '?' . $u[$s] . '&amp;page=' . $tt
                        . $mysearch . '" title="'
                        . $plugin_tx[$plugin]['page_label'] . ' ' . $tt . '">['
                        . $tt . ']</a>' . $separator;
                }
                $t .= "\n" . '</div>' . "\n";
            }

            if (!@$_REQUEST['realblog_story']
            ...
            for ($record_index=$start_index; $record_index <= $end_index;
                ...
                if ($record_index > $db_total_records - 1) {
                    ...
                    if (strstr($field[REALBLOG_HEADLINE], '|' . $realBlogCat . '|')
                        || strstr($field[REALBLOG_STORY], '|' . $realBlogCat . '|')
                        || $realBlogCat == 'all'
                        || ($realblogaction == "search"
                        && strstr($field[REALBLOG_H], '|' . $realBlogCat . '|'))
                    ) {
                        if ($pcf['teaser_multicolumns'] == 'true') {
                            $t .= "\n"
                        ...
                        if ($plugin_cf['realblog']['show_read_more_link'] == 'true'
                            ...
                            // shows number of comments in entries overview
                            if (function_exists('comments_nr')
                                && $pcf['comments_function'] == 'true'
                                && $field[REALBLOG_COMMENTS]
                            ) {
                                $realblog_comments_id = 'comments'
                                    . $field[REALBLOG_ID];
                                $t .= '<p class="realblog_number_of_comments">'
                                    . comments_nr($realblog_comments_id) . '</p>'
                                    . "\n";
                            }
...
* @global array  The paths of system files and folders.
* @global string The current languages.
* @global array  The contents of the pages.
* @global int    The current page index.
* @global mixed  FIXME
*
...
function Realblog_rss_adv($options = null)
{
    ...
    $tt = tag('br') . '<a href="' . $rss_path . 'realblog_rss_feed.xml">'
        . tag(
            'img src="' . $plugin_images_folder . 'rss.png" alt="'
            . $plugin_tx[$plugin]['rss_tooltip'] . '" style="border: 0;"'
        )
        . '</a>';
